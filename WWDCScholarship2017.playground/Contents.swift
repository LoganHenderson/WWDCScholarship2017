...
Merge Sort is a very efficient sorting algorithm with a worst case run time of O(nLog(n)). Merge sort is a recursive algorithm which means that it calls itself with a smaller input value. Merge sort works by using a devide and conquer strategy. Merge sort will break a list of n elements in half repeatedly it has n individual lists that each contain one item. Afterwards it will merge the broken down lists back together one at a time until you have the list split into two sorted halfs. At that point both halfes will merge into one fully sorted list.
